#!/usr/bin/env python3
"""
The Scroll â€” Daily AI & Social Media Marketing Newsletter Bot
...
# â”€â”€ Load config â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
load_dotenv()
...
def check_config():
    issues = []
    if not ANTHROPIC_API_KEY: issues.append("  â€¢ ANTHROPIC_API_KEY missing")
    if not SERPER_API_KEY:    issues.append("  â€¢ SERPER_API_KEY missing")
    if not GHL_API_KEY:       issues.append("  â€¢ GHL_API_KEY missing")
    if not GHL_LOCATION_ID:   issues.append("  â€¢ GHL_LOCATION_ID missing")
    if not GHL_USER_ID:       issues.append("  â€¢ GHL_USER_ID missing")
    if issues:
        print("\nâŒ Config issues found in .env:")
        for i in issues: print(i)
        sys.exit(1)


# â”€â”€ News Fetching â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
def fetch_news():
    print("ğŸ“° Fetching today's news...")
    queries = [
...
# â”€â”€ Newsletter Generation â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
def generate_newsletter(articles):
    print("âœï¸  Writing newsletter with Claude...")
    news_text = "\n\n".join([
        f"- {a['title']} ({a['source']})\n  {a['snippet']}"
        for a in articles if a['title']
    ]) or "No live news today â€” write a powerful evergreen issue instead."

    today    = datetime.now().strftime("%A, %B %d, %Y")

    prompt = f"""You are the writer behind "The Scroll" â€” the most valuable daily newsletter
...
---STORY---
3-4 sentences. Give real context â€” not just what happened, but WHY it's a big deal in AI and marketing. Make it feel significant.

---STORY_MEANS---
2-3 sentences that directly connect this news to the reader's reality as a small business owner or marketer. Be specific and concrete â€” not generic advice.
...
---CONTENT_IDEA---
Line 1: The concept/angle â€” what the post is about (1 sentence)
Line 2: Platform + format (e.g., "Instagram Reel" or "Facebook carousel post")
Line 3: Caption opener they can steal â€” first 1-2 sentences of the actual caption (make it compelling and specific)

---AI_PROMPT---
Try this in ChatGPT or Claude:
"[write a specific, immediately useful prompt for a small business owner â€” Name the context, the goal, and any constraints to make the output great]"
...
---AI_TOOL---
**[Tool Name]** â€” [one sentence: what it does and who it's for]
...
---CLOSE---
2 sentences max. Warm personal sign-off from Jayson. Ends with: â€” Jayson
...
- Action steps must name the specific tool, platform, and move â€” No vague advice
- Write for someone reading on their phone between meetings"""
    ...
    try:
        ...
        print(f"  âœ… Newsletter written ({len(content)} chars, stop_reason={message.stop_reason})", flush=True)
        return content
    except anthropic.APIConnectionError as e:
        print(f"\nâŒ Claude API connection error: {e}", flush=True)
        traceback.print_exc()
        sys.exit(1)
    except anthropic.RateLimitError as e:
        print(f"\nâŒ Claude API rate limit hit: {e}", flush=True)
        traceback.print_exc()
        sys.exit(1)
    except anthropic.APIStatusError as e:
        print(f"\nâŒ Claude API error {e.status_code}: {e.message}", flush=True)
        traceback.print_exc()
        sys.exit(1)
    except Exception as e:
        print(f"\nâŒ Unexpected error calling Claude: {e}", flush=True)
        traceback.print_exc()
        sys.exit(1)


# â”€â”€ Section Parser â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
# â”€â”€ HTML Builder â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
def build_html(newsletter_text):
    ...
    close_text  = s.get("CLOSE","Stay sharp out there. â€” Jayson")
...
# â”€â”€ Subscribers â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
THE_SCROLL_TAG = "the-scroll"

def get_subscribers_from_ghl():
    """Pull all contacts tagged 'the-scroll' directly from GHL â€” fully automatic."""
...
def tag_contact_in_ghl(contact_id):
    ...
    try:
        ...
        if resp.status_code in (200, 201):
            print(f"  âœ… Tagged contact {contact_id} as '{THE_SCROLL_TAG}'")
            return True
        else:
            print(f"  Warning: Could not tag contact â€” {resp.status_code}: {resp.text[:100]}")
    except Exception as e:
...
def get_subscribers_from_form():
    ...
    Tags each contact with 'the-scroll' automatically â€” no GHL workflow needed.
...
def get_subscribers():
    """
    Primary: fetch live from GHL by tag (fully automatic â€” dnew sign-ups included).
    ...
    """
    print("  ğŸ“‹ Fetching subscribers from GHL (tag: 'the-scroll')...")
    subs = get_subscribers_from_ghl()

    # Also pull from Newsletter form â€” auto-tags each submitter going forward
    print("  ğŸ“ Checking Newsletter form submissions for new subscribers...")
    ...
    if form_subs:
        ...
        for fs in form_subs:
            if fs["email"].lower() not in existing_emails:
                ...
                new_count += 1
        if new_count:
            print(f"  â– Added {new_count} new subscriber(s) from form submissions")
    ...
    print("  âš ï¸  No GHL subscribers found â€” falling back to subscribers.json")
    if not os.path.exists(SUBSCRIBERS_FILE):
        print("  No subscribers.json found either.")
        print("  Add: python3 newsletter_bot.py add \"First\" \"Last\" \"email\" \"ght_contact_id\"")
        return []
...
# â”€â”€ Send â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€*def send_newsletter(html_body, subscribers, subject):
    if not subscribers:
        print("No subscribers â€” nothing to send.")
        return 0

    email_from = f"{FROM_NAME} <{EROM_EMAIL}>"
    headers = {
        "Authorization": f"Bearer {GHL_API_KEY}",
        "Version":       "2021-07-28",
        "Content-Type":  "application/json"
    ...
    sent = 0
    print(f"ğŸ“¤ Sending to {len(subscribers)} subscriber(s)...")
    ...
    for sub in subscribers:
        ...
        try:
            ...
            if resp.status_code in (200, 201):
                print(f"  âœ… {sub['email']} â€” sent!")
                sent += 1
            else:
                print(f"  âŒ {sub['email']} â€” {resp.status_code}: {resp.text[:300]}")
        except Exception as e:
            print(f"  âŒ {sub['email']} â€” error: {e}")
...
def add_subscriber(first, last, email, contact_id):
    ...
    if not tagged:
        print("  âš ï¸  Could not tag in GHL â€” adding to local file only")
    ...
    print(f"âœ… Added {first} {last} ({email}) â€” tagged '{THE_SCROLL_TAG}' in GHL")
...
if __name__ == "__main__":
    ...
    print("\nâ•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•", flush=True)
    print("   THE SCROLL â€” Newsletter Bot v4 (Mobile-First)", flush=True)
    print(f"   {datetime.now().strftime('%Y-%m-%d %H:%M')}", flush=True)
    print("â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â